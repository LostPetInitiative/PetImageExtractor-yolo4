...
def main(_argv):
    ...
    while True:        
        ...
        if job is None:
            # uloading the model does not free the memory. thus commenting this
            ...
            if job['animal'] == "cat":
                ...
                log("{0}: unknown animal type: {1}".format(uid, job['animal']))
                continue
            ...
            bestScores = []
            rotations = []
            for (annotatedImage, extracted, bestScore, bestRotation) in rotationsJobs:
                if annotatedImage is None:
                    log("Did not find pet of proper type on one of the photos")
                    continue
                log("{0}: Found {1} with confidence {2:.4f} at rotation {3}".format(uid, job['animal'], bestScore, bestRotation))
