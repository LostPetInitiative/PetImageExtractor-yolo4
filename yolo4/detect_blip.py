...
def main(_argv):
    ...
    input_dir = _argv[1] # e.g. dev/found/synthetic_lost
    output_dir = _argv[2] # e.g. dev_extracted/found/synthetic_lost
    log(f"input: {input_dir}; output: {output_dir}")
    adDirs = os.listdir(input_dir)
    log(f"{len(adDirs)} ads to process")

    infer = None
    log("Executing eagerly: {0}".format(tf.executing_eagerly()))

    for adDir in tqdm(adDirs, ascii=True):
        fullAdDir = os.path.join(input_dir, adDir)
        imageFiles = [os.path.join(fullAdDir,x) for x in os.listdir(fullAdDir) if x.endswith("jpg") or x.endswith("png") or x.endswith("jpeg")]
        imagesNp = [io.imread(x) for x in imageFiles]

        metadata = json.load(open(os.path.join(fullAdDir, "card.json")))
        ...
        def FindBestRotation(npImage, targetClass):
            """Returns (annotatedImage,extractedPetImage, bestScore, bestRotation) or quadro-tuple on Nones if the pet is not detected"""
            original_image, image_data = loadImage(npImage)
            loadImageJobs = [rotateImageExt(image_data, k) for k in range(8)]
            bestScore = 0.0
            bestScoreBboxes = None
            noRotationValid = False
            spoiled = False
            bestRotation = 0
            for images_data, rotation in loadImageJobs:
                bboxes = getBboxes(images_data, targetClass)
            ...
            if not(bestScoreBboxes is None):
                ...
                return (extracted, bestScore, bestRotation)
                #return (None, None, None, None)
            else:
                return (None, None, None)
        
        if metadata['animal'] == "2":
            targetClass = 15
        elif metadata['animal'] == "1":
            targetClass = 16
        else:
            log("{0}: unknown animal type: {1}".format(adDir, metadata['animal']))
            continue

        rotationsJobs = [FindBestRotation(npImage, targetClass) for npImage in imagesNp]
        
        foundPetImages = []
        annotatedImages = []
        bestScores = []
        rotations = []
        for (extracted, bestScore, bestRotation) in rotationsJobs:
            if extracted is None:
                log("Did not find pet of proper type on one of the photos")
                continue
            log("{0}: Found {1} with confidence {2:.4f} at rotation {3}".format(adDir, metadata['animal'], bestScore, bestRotation))
        ...
        log("{0}: Detected needed pet ({2}) on {1} images out of {3} initial images".format(adDir, len(rotations), metadata['animal'], len(imagesNp)))

        adOutDir = os.path.join(output_dir,adDir)
        os.makedirs(adOutDir, exist_ok=True)
        for i in range(len(foundPetImages)):
            io.imsave(os.path.join(adOutDir,f"{i}.jpg"), foundPetImages[i])
            np.save(os.path.join(adOutDir,f"{i}.npy"), foundPetImages[i])
